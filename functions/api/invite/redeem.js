...
export async function onRequestPost({ request, env }) {
  ...
  if (!invite) { await logFail('invalid'); return json({ ok: false, code: 'INVALID' }, 403); }
  if (invite.status !== 'active') { await logFail('disabled'); return json({ ok: false, code: 'DISABLED' }, 403); }
  if (invite.expires_at && new Date(invite.expires_at).getTime() <= Date.now()) { await logFail('expired'); return json({ ok: false, code: 'EXPIRED' }, 403); }
  if (Number(invite.used_count) >= Number(invite.max_uses)) { await logFail('used_up'); return json({ ok: false, code: 'USED_UP' }, 403); }

  const newUsed = Number(invite.used_count) + 1;
  const nextStatus = newUsed >= Number(invite.max_uses) ? 'exhausted' : 'active';
  await DB.prepare(`UPDATE invitation_codes SET used_count = ?, status = ?, updated_at = ? WHERE id = ?`)
